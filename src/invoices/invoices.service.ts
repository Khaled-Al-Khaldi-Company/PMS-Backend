...
import { normalizeExecutionInput } from './invoice-billing.util';
...
export class InvoicesService {
      ...
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          for (const item of executionData) {
            ...
            if (!boqItem)
              throw new BadRequestException(
                `BOQ Item ${item.boqItemId} not found`,
              );
            ...
            if (
              ...
              const cItem = contract.items.find(
                (ci) => ci.boqItemId === boqItem.id,
              );
              if (!cItem)
                throw new BadRequestException(
            ...
            const previousQty = itemPrevDetails.reduce(
              (sum, d) => sum + d.currentQty,
              0,
            );

            let normalized;
            try {
              normalized = normalizeExecutionInput(
                item,
                boqItem,
                invoiceMaxQuantity,
                invoiceUnitPrice,
            ...
            const { currentQty, entryMode, currentProgressPercent } =
              normalized;
            if (currentQty <= 0) continue;

            const totalQty = previousQty + currentQty;

            if (totalQty > invoiceMaxQuantity + 1e-9) {
              throw new BadRequestException(
                `Executed quantity exceeds planned for BOQ item ${boqItem.itemCode || boqItem.description}`,
              );
            }
            ...
            detailsToCreate.push({
              ...
              currentValue,
              entryMode,
              currentProgressPercent: currentProgressPercent ?? null,
            });
          ...
          const retentionAmount =
            grossAmount * (contract.retentionPercent / 100.0);
          ...
          const taxAmount = Math.max(
            0,
            taxableAmount * (Number(taxPercent) / 100.0),
          );
  ...
  async revertToDraft(invoiceId: string) {
    ...
    if (invoice.status === 'PAID')
      throw new BadRequestException('لا يمكن إرجاع مستخلص تم دفعه إلى مسودة.');
    if (invoice.status === 'DRAFT')
      throw new BadRequestException('المستخلص بالفعل في حالة مسودة.');
  ...
  async deleteMustaqlasa(id: string) {
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (!invoice) throw new BadRequestException('المستخلص غير موجود');
        if (invoice.status !== 'DRAFT')
          throw new BadRequestException('لا يمكن حذف مستخلص معتمد أو مدفوع.');
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (existingInvoice.status !== 'DRAFT')
          throw new BadRequestException(
            'Cannot edit a certified or paid invoice',
          );
        ...
        for (const item of executionData) {
          ...
          const { currentQty, entryMode, currentProgressPercent } = normalized;
          if (currentQty <= 0) continue;
