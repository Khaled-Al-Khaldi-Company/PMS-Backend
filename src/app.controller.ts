...
export class AppController {
  ...
  async getDashboardStats() {
    ...
    const revenueInvoices = invoices.filter(
      (i) => i.contract?.type === 'MAIN_CONTRACT',
    );
    const certifiedRevenue = revenueInvoices
      .filter((i) => i.status === 'CERTIFIED')
    ...
    const totalCosts =
      costInvoices.reduce((acc, curr) => acc + Number(curr.netAmount), 0) +
    ...
    const allActivities = [
      ...invoices.map((inv) => ({
        id: inv.id,
        type:
          inv.contract?.type === 'MAIN_CONTRACT' ? 'REVENUE' : 'SUBCONTRACT',
    ...
    return {
      ...
      profitMargin:
        certifiedRevenue > 0
          ? ((certifiedRevenue - totalCosts) / certifiedRevenue) * 100
          : 0,
      totalSubcontractors,
      chartData,
